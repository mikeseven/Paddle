...
class BatchNormKernel<platform::GPUPlace, T> : public framework::OpKernel<T> {
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    CUDNN_ENFORCE(platform::dynload::miopenCreateTensorDescriptor(&data_desc_));
    CUDNN_ENFORCE(
    ...
    CUDNN_ENFORCE(platform::dynload::miopenSet4dTensorDescriptor(
        data_desc_, CudnnDataType<T>::type,
        dims.data()[0], dims.data()[1], dims.data()[2], dims.data()[3]));

    CUDNN_ENFORCE(platform::dynload::miopenDeriveBNTensorDescriptor(
    ...
    if (is_test) {
      ...
      CUDNN_ENFORCE(platform::dynload::miopenBatchNormalizationForwardInference(
          handle,
          // Note: PERSISTENT not implemented for inference
          miopenBNSpatial, (void*)CudnnDataType<T>::kOne(),
          (void*)CudnnDataType<T>::kZero(), data_desc_, x->template data<T>(),
          data_desc_, y->template mutable_data<T>(ctx.GetPlace()),
          bn_param_desc_, (void*)scale->template data<T>(), (void*)bias->template data<T>(),
          (void*)est_mean->template data<T>(), (void*)est_var->template data<T>(), epsilon));
    } else {
      // Run training mode.
      ...
      CUDNN_ENFORCE(platform::dynload::miopenBatchNormalizationForwardTraining(
          handle, mode_, (void*)CudnnDataType<T>::kOne(), (void*)CudnnDataType<T>::kZero(),
          data_desc_, (const void*)x->template data<T>(), data_desc_,
          y->template mutable_data<T>(ctx.GetPlace()), bn_param_desc_,
          (void*)scale->template data<T>(), (void*)bias->template data<T>(), this_factor,
    ...
    // clean when exit.
    CUDNN_ENFORCE(platform::dynload::miopenDestroyTensorDescriptor(data_desc_));
    CUDNN_ENFORCE(
...
class BatchNormGradKernel<platform::GPUPlace, T>
 ...
 public:
  void Compute(const framework::ExecutionContext &ctx) const override {
    ...
    CUDNN_ENFORCE(platform::dynload::miopenBatchNormalizationBackward(
